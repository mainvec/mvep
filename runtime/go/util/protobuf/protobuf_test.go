...
func TestProtobuffHttpHandelr(t *testing.T) {
	...
	encoder, ok := enc.LookupEncoding("protobuf")
	if !ok {
		t.Fatal("not ok")
	}
	...
	req.Header.Set("x-mainvec-cmd", "HubCreateCmd")
	...
	if respType != "application/x-protobuf" {
		t.Fatalf("content type mismatch, wanted %v, got %v", "application/x-protobuf", respType)
	}
...
func TestMap(t *testing.T) {
	e, ok := enc.LookupEncoding("protobuf")
	if !ok {
		t.Fatal("not ok")
	}
