...
func ExecuteGenerate(ctx context.Context, cmdIn string, cmdOutdir string, cmdLang string, skipCmd bool) error {

	bucket := validate.NewBucket()
	bucket.Validate("in", cmdIn, validate.NotBlank)
	bucket.Validate("lang", cmdLang, validate.NotBlank, validate.OneOfRule("go", "js"))
